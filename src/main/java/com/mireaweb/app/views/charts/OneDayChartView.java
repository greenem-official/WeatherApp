...
public class OneDayChartView extends VerticalLayout {
    private DateService dateService = new DateService();

    public OneDayChartView() {
        Panel panel = new Panel("График за день");
        panel.setSizeUndefined();
        long userId = UserState.get().getUser().getId();

        List<Weight> weights = dateService.selectLastWeek(userId);
        //System.out.println(weights.toString());
        Date today = DateUtil.today();
        panel.setContent(new ChartView("Индекс массы тела", WeightUtil.toRange(weights, today, today, 0)));
